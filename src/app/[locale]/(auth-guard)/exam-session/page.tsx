...
const ExamSimulator = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/exam/topics?file=${filename}`, {
          headers: { Authorization: `Bearer ${session.data.session?.access_token}` },
        });

        if (res.ok) {
          const fetchedTopics = await res.json();
          const formattedTopics = fetchedTopics.map((topic: { topic: string; description: string; }) => ({
            topic: {
              topic: topic.topic,
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen">
      {loading ?
        <div className="mt-auto mb-auto bg-red-400">
          <p className="text-lg font-semibold">Processing your document and preparing exam questions, please wait.</p>
        </div>
        : (
          <>
              ...
              <div className="flex space-x-4 items-center">
                ...
                {currentRole === null && isCurrentTopicComplete() && (
                  <button onClick={handleNextTopic} className="bg-green-500 hover:bg-green-600 text-dark font-semibold px-4 py-2 rounded-lg">
                    {currentRound < rounds.length - 1 ? "Next Topic" : "Submit and Get Results"}
                  </button>
                )}
              </div>
            </div>
                ...
                {currentRound < rounds.length && (
                  <>
                    <h2 className="text-xl font-semibold">
                      {currentRole === null
                        ? "Press Start to reveal the topic"
                        : `Topic: ${rounds[currentRound].topic.topic}`}
                    </h2>
                    {currentRole !== null && <p className="text-sm text-gray-500">
                      {rounds[currentRound].topic.description}
                    </p>}
                  ...
                  <div>
                    ...
                    {rounds.map((round, index) => (
                      <div key={index} className="mb-4">
                        <h3 className="text-lg font-semibold">Round {index + 1}: {round.topic.topic}</h3>
